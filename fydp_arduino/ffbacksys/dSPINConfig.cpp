//CODE MODIFIED FOR L6472 COMPATABILITY BY JARON SZPINDEL 
//ORIGINAL AUTHOR https://github.com/braingram/dSPIN
#include "dSPIN.h"
...
// The TVAL registers are...weird. I don't entirely understand how they differ
//  from the microstepping, but if you have trouble getting the motor to run,
//  tweaking TVAL has proven effective in the past. There's a separate register
//  for each case: running, static, accelerating, and decelerating.

void dSPIN::setAccTVAL(byte tvalInput, byte index)
{
  setParam(TVAL_ACC, tvalInput, index);
}

void dSPIN::setAccTVAL(byte tvalInput)
{
  setParam(TVAL_ACC, tvalInput);
}

byte dSPIN::getAccTVAL(byte index)
{
  return (byte) getParam(TVAL_ACC, index);
}

void dSPIN::setDecTVAL(byte tvalInput, byte index)
{
  setParam(TVAL_DEC, tvalInput, index);
}

void dSPIN::setDecTVAL(byte tvalInput)
{
  setParam(TVAL_DEC, tvalInput);
}

byte dSPIN::getDecTVAL(byte index)
{
  return (byte) getParam(TVAL_DEC, index);
}

void dSPIN::setRunTVAL(byte tvalInput, byte index)
{
  setParam(TVAL_RUN, tvalInput, index);
}

void dSPIN::setRunTVAL(byte tvalInput)
{
  setParam(TVAL_RUN, tvalInput);
}

byte dSPIN::getRunTVAL(byte index)
{
  return (byte) getParam(TVAL_RUN, index);
}

void dSPIN::setHoldTVAL(byte tvalInput, byte index)
{
  setParam(TVAL_HOLD, tvalInput, index);
}

void dSPIN::setHoldTVAL(byte tvalInput)
{
  setParam(TVAL_HOLD, tvalInput);
}

byte dSPIN::getHoldTVAL(byte index)
{
  return (byte) getParam(TVAL_HOLD, index);
}
